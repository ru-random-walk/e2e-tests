...
class PersonScheduleTest extends MatcherTest {
    ...
    public void addUserAvailableTime() {
        var clubId = personClubFunctions.getUserClubs(userConfigService.getUserByRole(UserRoleEnum.TEST_USER).getUuid()).get(0);
        try {
            availableTimeMatcherApi.addAvailableTime(clubId, OffsetTime.now(), OffsetTime.now().plusHours(1), LocalDate.now());
        } catch (Exception ignored) {
        }
        try {
            availableTimeMatcherApi
                    .addAvailableTime(clubId, OffsetTime.now().plusHours(2), OffsetTime.now().plusHours(4), LocalDate.now());
        } catch (Exception ignored) {
        }
        try {
            availableTimeMatcherApi.addAvailableTime(
                    clubId,
                    OffsetTime.now().plusHours(1),
                    OffsetTime.now().plusHours(2),
